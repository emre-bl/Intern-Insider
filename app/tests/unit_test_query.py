...
class TestBuildReviewsQuery(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        data_file_path = os.path.join(PROJECT_ROOT, "app/tests/data/mydatabase.reviews.json")
    ...
    def test_internship_role_filter_with_data(self):
        ...
        expected_result = [
            review for review in self.test_reviews
            if "Data Analyst Intern".lower() in review["internship_role"].lower() and review["admin_approved"] == True
        ]
        ...
        matched_reviews = [
            ...
            if all(
                (
                    isinstance(query[key], dict) and
                    "$regex" in query[key] and
                    re.search(query[key]["$regex"], review[key], re.IGNORECASE)
                ) if isinstance(query[key], dict) else
                (query[key] == review[key])
                for key in query if key in review and query[key] is not None
            )
        ]
    ...
    def test_sort_reviews(self):
        ...
        self.assertEqual(sorted_reviews[0]["like_count"], max_like_count, "Most liked review is not at the top")
        self.assertEqual(sorted_reviews[-1]["like_count"], min_like_count, "Least liked review is not at the bottom")

        # Newest First
        sorted_reviews = sort_reviews(self.test_reviews, "Newest First")
        for i in range(1, len(sorted_reviews)):
            current_date = datetime.strptime(sorted_reviews[i]["feedback_date"], "%d/%m/%Y")
            previous_date = datetime.strptime(sorted_reviews[i - 1]["feedback_date"], "%d/%m/%Y")
            self.assertGreaterEqual(previous_date, current_date, "Reviews are not sorted by newest first")
        ...
        for i in range(1, len(sorted_reviews)):
            current_date = datetime.strptime(sorted_reviews[i]["feedback_date"], "%d/%m/%Y")
            previous_date = datetime.strptime(sorted_reviews[i - 1]["feedback_date"], "%d/%m/%Y")
            self.assertLessEqual(previous_date, current_date, "Reviews are not sorted by oldest first")
